...
## 프로젝트 개요 및 배경
비대면 쇼핑과 중고 물품 구매·대여 수요가 증가하면서 중고 시장이 빠르게 성장하고 있다. 지속 가능한 소비와 경제적 이유로 중고 거래 선호도가 높아졌고, AI 기술의 발전으로 개인 맞춤형 서비스의 중요성이 커졌다.<br>
...
이메일 & 비밀번호 기반 로그인<br>
아이디 찾기: 이메일 인증 후 아이디 제공<br>
비밀번호 찾기: 이메일 인증 후 임시 비밀번호 발급<br>
회원가입: 3단계 진행 (정보 입력 → 이메일 인증 → 프로필 설정)<br>
아이디/닉네임 중복 검사<br>
비밀번호 정규식 검사 (영문+숫자+특수문자 8자 이상)<br>
주소 검색 API 활용<br>
프로필 사진 업로드<br>

### 2. 검색 기능
키워드 검색: 제목, 설명, 카테고리 기반 필터링<br>
이미지 검색: 이미지 업로드 시 배경 제거 및 YOLOv8 기반 카테고리/이름 분류 (FastAPI 서버를 활용하여 처리)<br>

### 3. 상품 등록
필수 입력 정보: 제목, 상품명, 이미지, 카테고리, 대여 가격, 대여 가능 기간, 설명<br>
자동 분류: YOLOv8을 이용하여 이미지 분석 후 상품명 & 카테고리 자동 입력<br>
유사 상품 가격 비교: 같은 카테고리 내 가격 분포 그래프 제공<br>

### 4. 거래 및 대여 시스템
상세 페이지: 상품 정보, 이미지, 동영상 확인<br>
채팅 기능: 실시간 메시지 & 중간 거래 장소 추천 기능<br>
대여 신청 프로세스: 신청 후 대여자가 "빌려주기" 승인 시 대여 확정 / 거래 종료 후 반납 확인 버튼 클릭 시 종료<br>

### 5. 숏폼 페이지
상품 홍보 영상 업로드 가능<br>
랜덤 재생 및 자동 소리 ON/OFF 기능 제공<br>

### 6. 중간거리 찾기
판매자 & 대여자의 주소 입력 시 자동 중간 지점 계산 후 추천<br>

### 7. 마이페이지 및 리뷰 시스템
내 정보 변경 (비밀번호, 주소, 닉네임, 프로필 사진 변경 가능)<br>
거래 내역 조회 및 리뷰 작성-별점 평가<br>
...
## 인공지능
### 1. YOLOv8 모델
YOLOv8은 객체 검출 모델로, 실시간 객체 인식 및 분류에 최적화된 격량 네트워크다. 본 프로젝트에서는 제품 등록 시 이미지 내의 객체를 신속하게 탐지하고 자동으로 분류하는데 사용된다. 앵커 프리(anchor-free) 방식을 채택하여 복잡한 배경에서도 안정적인 인식 성능을 보이며, 빠른 추론 속도와 높은 정확도를 통해 다양한 제품 카테고리를 분류하는 데 적용할 수 있다.

### 2. ResNet50 모델
ResNet50은 50개의 층으로 구성된 심층 잔차 네트워크로, 이미지 분류와 특징 추출 분야에서 강력한 성능을 가진다. 잔차 연결(Residual Connection)을 통해 매우 깊은 네트워크에서도 효과적으로 학습할 수 있으며, 이를 통해 제품 이미지의 세밀한 특징을 추출할 수 있다. 본 모델은 사용자가 업로드한 제품 이미지와 등록된 상품의 이미지 간 유사도 비교에 활용되며, 유사 이미지 검색 기능을 구현하는 핵심 기능이다.

### 3. 크롤링

본 프로젝트에서는 Selenium을 활용하여 웹 크롤링을 수행했다. Selenium은 동적 웹페이지에서도 자동으로 데이터를 수집할 수 있는 라이브러리로, 본 프로젝트에서는 쿠팡의 제품 이미지를 크롤링하는 데 사용했다.

### 4. 데이터셋
본 프로젝트에서는 Roboflow 사이트를 활용하여 크롤링한 쿠팡 제품 이미지의 전처리를 진행했다. 이미지 라벨링과 이미지 증강 등의 과정을 거쳐 서 학습 데이터를 생성했다. 이 과정을 통해 모델의 일반화 능력을 높이고, 실제 서비스 환경에서 높은 인식 성능을 기대할 수 있다.

### 5. 모델 정확도
|  | **Precision** | **Recall** | **mAP50** |
| :--- | :---: | :---: | :---: |
| **의자** | 94.5% | 95.9% | 96.5% |
| **아이스박스** | 94.7% | 92.9% | 96.1% |
| **텐트** | 95.5% | 93.8% | 98.1% |
| **카메라** | 95.6% | 84.9% | 96.9% |
| **노트북**| 97.5% | 97.6% | 98.2% |
| **스마트폰** | 90.4% | 91.1% | 94.2% |
...
+ **Precision(정확도)**
  <br>
  감지된 개체의 정확도로, 얼마나 많은 개체를 정확하게 감지했는지를 나타내는 값.
+ **Recall(리콜)**
  <br>
  이미지에서 객체의 모든 인스턴스를 식별하는 모델의 기능.
+ **mAP50**
  <br>
  평균 평균 정밀도(IoU) 임계값 0.50에서 계산된 평균 정밀도. '쉬운' 탐지만 고려한 모델의 정확도를 측정한 값.
